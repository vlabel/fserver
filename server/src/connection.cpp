                             ...
                             std::size_t bytes_transferred)
{
    ...
    if ((request_.file_size != 0) and (request_.data )) {
        std::shared_ptr<std::vector<char>> sh(request_.data.release());   // shared from uniq and later send it to io_thread
        if (!storage_h.is_inited())  storage_h.init(request_.file_size,request_.file_name,directory_);
        storage_h.write(shared_from_this(),sh);                           // send to disk
